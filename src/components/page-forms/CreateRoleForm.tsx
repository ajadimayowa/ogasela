...
const CreateRoleForm = () => {
    ...
    const handleSubmit = async (payload: ILogin) => {
        ...
        try {
            const res = await api.post('role/create', payloadData);
            // navigate('/root-login');
    ...
    const getDepartments = async () => {
        try {
            const res = await api.get(`/departments?organizationId=${staffProfile?.organization}`);
            if (res.status == 200) {
